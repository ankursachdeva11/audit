...
$(document).on('pageinit', '#pgSignIn', function(){
	...
	$("#submit").click(function(){
	   ...
	   $.ajax({
		   type: "GET",
		   url: "https://www.qhse-riskapplications.com/m/checklogin.php",
		   cache: false,
		   dataType: "text",
		   data: formData,
		   beforeSend: function() {
...
$(document).on('pageinit', '#pgSignUp', function(){
	...
	$("#frmSignUpRegister").click(function(){
	   ...
	   $.ajax({
		   type: "GET",
		   url: "https://qhse-riskapplications.com/m/registerUser.php",
		   cache: false,
		   dataType: "text",
		   data: formData,
		   beforeSend: function() {
...
$(document).on('pagebeforeshow', '#pgProjects', function(){
	...
	$.ajax({url: "https://www.qhse-riskapplications.com/m/show_projects.php?uid=" + value + "&mode=" + modeValue,
		dataType: "json",
		cache: false,
...
$(document).on('pagebeforeshow', '#pgRegister', function(){
	...
	$.ajax({url: "https://qhse-riskapplications.com/m/getCompanyDetails.php?uid=" + value,
		dataType: "json",
		cache: false,
		jsonpCallback:'successCallback',
		async: true,
	...
	function showCompanyDetails(result) {
		...
		$.each(objCompanies, function(i, values) {
			...
			$.ajax({url: "https://qhse-riskapplications.com/m/getCountryList.php",
				dataType: "json",
				cache: false,
				jsonpCallback:'successCallback',
				async: true,
...
$(document).on('click','#btnLostPassword',function(){
   ...
   var formData = $("#frmForgotPassword").serialize();
	   ...
	   $.ajax({
		   type: "GET",
		   url: "https://www.qhse-riskapplications.com/m/lostpassword.php",
		   cache: false,
		   dataType: "text",
		   data: formData,
		   beforeSend: function() {
...
$(document).on('pagebeforeshow', '#pgModules', function(){
		...
		//alert(modeValue);
		//alert(value);
	$.ajax({url: "https://qhse-riskapplications.com/m/show_modules.php?fid=0&pid=" + strProjectId + "&mode=" + modeValue + "&uid=" + value,
		dataType: "json",
		jsonpCallback: 'successCallback',
		async: true,
		beforeSend: function() {
		...
		if (objModules.length == 0) {
			...
			$.each(objModules, function(i, values) {
				...
				htmlContent = htmlContent + '<li data-role="fieldcontain" style="white-space:normal">';
						...
						htmlContent = htmlContent + '<span class="ui-li-count">' + objModules[i].limits + '</span>';
						...
						htmlContent = htmlContent + '<a href="#pgPurchaseProducts">';
						//htmlContent = htmlContent + objModules[i].sub_module_name + ' :: ' + objModules[i].form_name;
						htmlContent = htmlContent + objModules[i].form_name;
						htmlContent = htmlContent + '<span class="ui-li-count">Buy Again</span>';
						htmlContent = htmlContent + '</a>';
...
$(document).on('pagebeforeshow', '#pgPurchaseProducts', function(){
	...
	$.ajax({url: "https://qhse-riskapplications.com/m/combine_purchase_products.php?uid=" + value,
		dataType: "json",
		jsonpCallback: 'successCallback',
		async: true,
		beforeSend: function() {
			...
			$.each(objPurchaseProduct, function(i, values) {
				...
				htmlContent = htmlContent + '<li data-role="fieldcontain" style="white-space:normal">';
					...
					//htmlContent = htmlContent + '<a  onclick="callPaypal()" href="#?product_id=' + objPurchaseProduct[i].product_id + '&project_id=' + objPurchaseProduct[i].project_id + '&price=' + objPurchaseProduct[i].product_price + '&form_id=' + objPurchaseProduct[i].product_form_id + '">';						
						htmlContent = htmlContent + '<a  onclick="callPaypal(\''+ objPurchaseProduct[i].product_name + '\',\''+ objPurchaseProduct[i].product_price + '\',\''+ objPurchaseProduct[i].product_form_id + '\',\''+ objPurchaseProduct[i].project_id + '\',\''+ value +'\',\'' + device.uuid +'\')"><h2>';
						...
						//console.log(objPurchaseProduct[i].free_trail);
						htmlContent = htmlContent + '</h2><p><img src="img/buy.png"/>&nbsp;';
						if(objPurchaseProduct[i].free_trail == 1){
						htmlContent = htmlContent + '<a href="javascript:void(0);"><img src="img/free.png" onclick="productId(' + objPurchaseProduct[i].product_id + ')"/></a></p></a>';
						}
...
$(document).on('pagebeforeshow', '#pgInprogressAndCompleted', function(){
	...
	$.ajax({url: "https://qhse-riskapplications.com/m/inprogressCompleted.php?pid=" + strProjectId + "&mode=" + modeValue + "&uid=" + value + "&mid=" + module_id,
		dataType: "json",
		jsonpCallback: 'successCallback',
		async: true,
		beforeSend: function() {
		...
		if (objModules.length == 0) {
			...
			$.each(objModules, function(i, values) {
				...
				if(i=='0'){
					...
					var htmlContent = '<li>';
					htmlContent = htmlContent + '<a href="#" onclick="displayPDF(\'https://qhse-riskapplications.com/generate_pdf/'+ objModules[i].pdfPage +'?pid='+ objModules[i].project_id +'&fid='+ objModules[i].form_id +'&did='+ objModules[i].did +'\')"><p> Initial Date : ' + objModules[i].timestamp +'</p><p> Published Date : '+objModules[i].completedTimestamp +'</p></a><a href="#" data-icon="delete" onclick="deleteProductInfo(' + objModules[i].project_id + ',' + objModules[i].form_id + ',' + objModules[i].did + ')">Some Text</a></li>';
					}
					else{
						...
						htmlContent = htmlContent + '<li data-role="fieldcontain" style="white-space:normal"><a href="#" onclick="displayPDF(\'https://qhse-riskapplications.com/generate_pdf/'+ objModules[i].pdfPage +'?pid='+ objModules[i].project_id +'&fid='+ objModules[i].form_id +'&did='+ objModules[i].did +'&inprogress=yes\')">Current Risk Score</a></li>';
...
function confirmDeleteProject(Button,pid){
	...
	$.ajax({url: "https://qhse-riskapplications.com/m/deleteProject.php?uid=" + value + "&mode=" + modeValue + "&pid=" + pid,
		dataType: "json",
		cache: false,
		jsonpCallback:'successCallback',
		async: true,
...
function confirmDeleteProductInfo(Button,pid,fid,did){
		...
		$.ajax({url: "https://qhse-riskapplications.com/m/deleteProductRecods.php?pid=" + pid + "&mode=" + modeValue + "&uid=" + value + "&mid=" + module_id + "&fid=" + fid + "&did=" + did,
		dataType: "json",
		jsonpCallback: 'successCallback',
		async: true,
		beforeSend: function() {
		...
		if (objModules.length == 0) {
			...
			$.each(objModules, function(i, values) {
				...
				if(i=='0'){
					...
					htmlContent = htmlContent + '<a href="#" onclick="displayPDF(\'https://qhse-riskapplications.com/generate_pdf/'+ objModules[i].pdfPage +'?pid='+ objModules[i].project_id +'&fid='+ objModules[i].form_id +'&did='+ objModules[i].did +'\')"><p> Initial Date : ' + objModules[i].timestamp +'</p><p> Published Date : '+objModules[i].completedTimestamp +'</p></a></li>';
					}
					else{
						var htmlContent = '<li data-theme="b"><a href="#">' + objModules[i].timestamp + '</a><a href="#" data-icon="delete" onclick="deleteProductInfo(' + objModules[i].project_id + ',' + objModules[i].form_id + ',' + objModules[i].did + ')">Some Text</a></li>';
						htmlContent = htmlContent + '<li data-role="fieldcontain" style="white-space:normal" ><a href="#pgSection?mid=' + objModules[i].module_id +'&pid=' + objModules[i].project_id + '&fid=' + objModules[i].form_id + '&did=' + objModules[i].did + '">';
						htmlContent = htmlContent + 'Inprogress' ;
						htmlContent = htmlContent + '</a></li>';
						htmlContent = htmlContent + '<li data-role="fieldcontain" style="white-space:normal"><a href="#" onclick="displayPDF(\'https://qhse-riskapplications.com/generate_pdf/'+ objModules[i].pdfPage +'?pid='+ objModules[i].project_id +'&fid='+ objModules[i].form_id +'&did='+ objModules[i].did +'\')">Current Risk Score</a></li>';
...
$(document).on('pagebeforeshow', '#pgSection', function(){
	...
	$.ajax({url: "https://www.qhse-riskapplications.com/m/show_sections.php?mid=" + strModuleId + "&pid=" + strProjectFormId + "&fid=" + strFormId + "&mode=" + modeValue,
		dataType: "json",
		jsonpCallback: 'successCallback',
		async: true,
		beforeSend: function() {
...
$(document).on('pagebeforeshow', '#pgAudit_Risk_Profiling', function(){
	//	strSectionId is coming from the router funtionc at the begining of this .js file
	 ...
	 alert("https://qhse-riskapplications.com/m/get_questions.php?section_id=" + strSectionId + "&userid=" + uid + "&projectid=" + strProjectId + "&moduleid=" + intModuleId + "&did=" +intDetailId);
	 $.ajax({url: "https://qhse-riskapplications.com/m/get_questions.php?section_id=" + strSectionId + "&userid=" + uid + "&projectid=" + strProjectId + "&moduleid=" + intModuleId + "&did=" +intDetailId,
		dataType: "json",
		jsonpCallback: 'successCallback',
		async: true,
		beforeSend: function() {
	...
	function getQuestions(result) {
			...
			$.each(objQuestion, function(i, values) {
				...
				if (objQuestion[i].section_title != '0') {
					if(objQuestion[i].section_id!=212 && objQuestion[i].section_id!=213 && objQuestion[i].section_id!=73 && objQuestion[i].section_id!=214 && objQuestion[i].section_id!=0 && objQuestion[i].section_id!=241 && objQuestion[i].section_id!=264 && objQuestion[i].section_id!=265 && objQuestion[i].section_id!=352 && objQuestion[i].section_id!=387){									
						$("#uiAudit_Risk_Profiling").append('<li data-role="list-divider">' + objQuestion[i].section_title + ' [' + objQuestion[i].section_id + ']</li>');
					}
					else if(objQuestion[i].section_id==212 || objQuestion[i].section_id==73 || objQuestion[i].section_id==0 || objQuestion[i].section_id==241  || objQuestion[i].section_id==352 || objQuestion[i].section_id==387){
						$("#uiAudit_Risk_Profiling").append('<li data-role="list-divider">' + objQuestion[i].section_title + '</li>');
					}
					else if(objQuestion[i].section_id==213 || objQuestion[i].section_id==214 || objQuestion[i].section_id==265 || objQuestion[i].section_id==353){
				...
				var htmlContent = '<li data-role="fieldcontain">';
						if (objQuestion[i].stage != "" ) {	//	if stage is empty, do not show them on devices
							if(objQuestion[i].section_id!=212 && objQuestion[i].section_id!=213 && objQuestion[i].section_id!=73 && objQuestion[i].section_id!=214 && objQuestion[i].section_id!=0 && objQuestion[i].section_id!=264 && objQuestion[i].section_id!=241 && objQuestion[i].section_id!=265 && objQuestion[i].section_id!=352 && objQuestion[i].section_id!=387){
						...
						if(objQuestion[i].section_id==212 || objQuestion[i].section_id==213 || objQuestion[i].section_id==214 || objQuestion[i].section_id==73 || objQuestion[i].section_id==0 || objQuestion[i].section_id==241 || objQuestion[i].section_id==264 || objQuestion[i].section_id==265 || objQuestion[i].section_id==352 || objQuestion[i].section_id==387){
												
						}
						else if((objQuestion[i].section_id>=187 && objQuestion[i].section_id<=211) || (objQuestion[i].section_id>=216 && objQuestion[i].section_id<=354 && objQuestion[i].section_id>=388)){
						...
						if(objQuestion[i].section_id!=212 && objQuestion[i].section_id!=213 && objQuestion[i].section_id!=73 && objQuestion[i].section_id!=214 && objQuestion[i].section_id!=241 && objQuestion[i].section_id!=264 && objQuestion[i].section_id!=265 && objQuestion[i].section_id!=352 && objQuestion[i].section_id!=387 && objQuestion[i].section_id!=0){
						...
						htmlContent = htmlContent + '<a href="#" data-role="button" data-mini="true" onclick="window.open(\'https://qhse-riskapplications.com/wp-content/themes/twentytwelve/uploads/' + objQuestion[i].imgUploadFolder + '/' + objQuestion[i].detail_id + '/' + objQuestion[i].media + '\',\'_system\',\'location=yes\')">View Evidence</a>';
	...
	/*
	 function getPicture(did,media)
            ...
            window.open("https://qhse-riskapplications.com/wp-content/themes/twentytwelve/uploads/iso_14001_2004_audit_checklist/" + did + "/" + media ,'_blank' , 'location=yes');
	...
	$("#btnTMSASubmit").off().on('click',function(e){
	   ...
	   var formData = $("#frmTMSASubmit").serialize();
		...
		$.ajax({
			type: "post",
			url: "https://qhse-riskapplications.com/m/save_questions_answers.php?is_publish=0&moduleid="+intModuleId + "&mode=" + varMode + "&did=" +intDetailId + "&currentTime=" + curTime,
			cache: false,
			dataType: "text",
			async: true,
			data: formData,
	...
	function onConfirm(button){
	  ...
	  $.ajax({
			type: "post",
			url: "https://qhse-riskapplications.com/m/save_questions_answers.php?is_publish=1&moduleid="+intModuleId + "&mode=" + varMode + "&did=" +intDetailId + "&currentTime=" + curTime,
			cache: false,
			dataType: "text",
			async: true,
			data: formData,
...
$(document).on('click','#btnRegister',function(e){
		...
		$.ajax({
			type: "post",
			url: "https://qhse-riskapplications.com/m/registerCompany.php?uid=" + uid,
			cache: false,
			dataType: "text",
			async: true,
			data: formData,
...
$(document).on('click','#btnFreeTrial',function(e){
		//alert("Form Data ");	
		var formData = $("#frmFreeTrial").serialize();
					
        var uid = window.localStorage.getItem("userid");
	   	var pid = window.localStorage.getItem("product_id");		
		$.ajax({
			type: "post",
			url: "https://qhse-riskapplications.com/m/assign_trial.php?uid=" + uid + "&pid=" + pid,
			cache: false,
			dataType: "text",
			async: true,
			data: formData,
...
function productId(pid){
		...
		window.localStorage.setItem('product_id',pid);
		$.ajax({
			url: "https://qhse-riskapplications.com/m/free_trial_processes.php?action=getproductname&pid=" + pid,
			cache: false,
			beforeSend: function() {
				$.mobile.showPageLoadingMsg(true);
			},
			...
			success: function (data) {
				$("#freeProductName").html(data);
				//location.href='#pgFreeTrial';
				$.mobile.changePage('#pgFreeTrial','slide');
			},
			error: function (request,error) {
                        // This callback function will trigger on unsuccessful action 
						//alert(request + " - " + error);
